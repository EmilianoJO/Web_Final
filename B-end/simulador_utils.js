...
function classIdToHTML(course) {
    if (course != '0') {
        return `<p>${course}</p><p><button class="botonDelete" onclick="eliminaMateria('${course}')">Eliminar</button></p>`;
    } else {
        return '';
    }
...
//delete materia al picar boton

function eliminaMateria(course) {
    let userIdlogeado = sessionStorage.getItem('newUser');
    userIdlogeado = JSON.parse(userIdlogeado);
    let url = `http://localhost:3000/schedule/${userIdlogeado.userId}`;

    let courseAEliminar = new Object();
    //courseAEliminar.classId = course;
    let urlCurso = `http://localhost:3000/courses/${course}`;
    console.log(urlCurso);
    //let courseFin;
    loadCourses(urlCurso).then(courseFinal => {
        console.log(courseFinal);
        courseAEliminar = courseFinal;
        //console.log(courseFinal);
        
        deleteCourses(url, courseAEliminar, curso => {
            console.log(JSON.stringify(courseAEliminar));
        }, (error) => console.log(error));
        
    });
    //console.log(courseAEliminar);
}
